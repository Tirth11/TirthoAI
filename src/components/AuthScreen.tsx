...
export function AuthScreen({ initialMode = "signup", onContinueAsGuest }: AuthScreenProps = {}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "signup") {
        // Friction-free registration: create an already-confirmed account via
        // the server (Admin API) — no confirmation email, so no "too many
        // attempts" email rate limit — then sign in with the password.
        ...
        if (!res.ok) {
          throw new Error(reg?.message || "Could not create your account.");
        }
        ...
        toast.success(
          reg?.error === "user_exists"
            ? "Welcome back!"
            : "Welcome! 500 free credits added to your account 🎉",
        );
      } else {
